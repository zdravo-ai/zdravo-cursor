...
async function api(path, method = 'GET', body, requiredScopes = ['mcp:personal']) {
...
function requireScopes(toolName) {
...
function formatBrandedOutput(name, data) {
    const emoji = {
        save_to_zdravo: '✨',
        search_zdravo: '🔍',
        get_zdravo_stats: '📊',
    ...
    if (d?.memories ?? d?.results) {
        const results = (d.memories ?? d.results ?? []);
        const items = results.map((m, i) => `  ${ZDRAVO_YELLOW}[${i + 1}]${RESET} ${String(m.title ?? m.id ?? '')} ${m.score ? `(${Math.round(Number(m.score) * 100)}%)` : ''}`).join('\n');
...
async function getScaffolding(opts) {
    ...
    const scored = Object.entries(SCAFFOLDING_TEMPLATES).map(([key, template]) => {
        ...
        if (filePath && template.code.includes(filePath.split('/').pop()?.split('.')[0] || ''))
            score += 3;
...
const server = new Server({ name: 'zdravo-mcp', version: '2.2.4' }, { capabilities: { tools: {}, resources: {} } });
